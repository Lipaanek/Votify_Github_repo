...
router.get('/api/login', async (req, res) => {
    ...
    if (await !dbInstance.doesUserExist(req.query.email)) {
        return res.status(400).json({ error: 'User does not exist' });
    }
    (0, auth_1.sendVerificationMail)(req.query.email);
...
router.get('/api/register', async (req, res) => {
    ...
    try {
        await dbInstance.addUser({ email: req.query.email, name: '' });
        (0, auth_1.sendVerificationMail)(req.query.email);
...
router.get('/api/login/code', (req, res) => {
    if (!dbExists()) {
        return;
    }
    ...
    (0, auth_1.validateCode)(email, code)
        .then(async (isValid) => {
        if (isValid) {
...
router.get('/api/info/groups', async (req, res) => {
    const email = req.query.email;
    if (!dbExists() || !email) {
        return;
    }
    const info = await dbInstance.getUserInfo(email);
    if (!info) {
...
router.get('/api/poll/:pollId', async (req, res) => {
    if (!dbExists()) {
        return;
    }
    const cookie = req.cookies.session;
...
router.post('/api/poll/:pollId/option', async (req, res) => {
    if (!dbExists()) {
        return;
    }
    const cookie = req.cookies.session;
...
router.get('/api/info/polls', async (req, res) => {
    const email = req.query.email;
    if (!dbExists() || !email) {
        return;
    }
...
router.get('/api/group/:groupId/polls', async (req, res) => {
    if (!dbExists()) {
        return;
    }
    const cookie = req.cookies.session;
...
router.get('/api/group/:groupId/info', async (req, res) => {
    if (!dbExists()) {
        return;
    }
    const cookie = req.cookies.session;
...
router.get('/api/group/:groupId/public', async (req, res) => {
    if (!dbExists()) {
        return;
    }
    const groupId = parseInt(req.params.groupId);
...
router.post('/api/group/:groupId/join', async (req, res) => {
    if (!dbExists() || !dbInstance.isReady()) {
        return res.status(500).json({ error: 'Database not ready' });
    }
    const cookie = req.cookies.session;
    if (!cookie) {
...
router.post('/api/poll/:pollId/vote', async (req, res) => {
    if (!dbExists()) {
        return;
    }
    const cookie = req.cookies.session;
